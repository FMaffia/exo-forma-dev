    ...
    return (
        <div>
            ...
            <MenuList>
                ...
                <MenuItem selected={location.pathname === STEP_FORM_PATH_NEW}
                          onClick={() => navigate(STEP_FORM_PATH_NEW)}>
                    <ListItemIcon>
                        <AddCircle color={'primary'}/>
                    </ListItemIcon>
                    <ListItemText>Aggiungi step</ListItemText>
                </MenuItem>
                {steps?.length > 0 && <Typography sx={{p: 2, fontWeight: 600}} variant={"button"} color={"inherit"}>
                    <span style={{color: purple[600]}}>Steps</span>
                </Typography>}
                {steps?.map(step => <ListItem selected={location.pathname === STEP_FORM_PATH + step.number}
                                              key={step.number}
                                              disablePadding>
